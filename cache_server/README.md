...
4. An automated jenkins production job (that emails buildcops upon failure) that
   logs into each server and checks it is running and has enough disk space is
   run daily.
...
7. Clone this repository to `/cache/drake-ci`:
    ...
    server under `/cache/drake-ci` affects a production system.  The cleanup
...
11. Confirm that `echo $USER` reveals you are `root`, and then execute
    ...
    0 22     * * *   /cache/drake-ci/cache_server/cleanup_old_files.py -s --days 3 /cache/data >>/cache/log/cleanup_old_files.log 2>&1
    #
    # Cache (over)utilization alerting: run 7am eastern.
    0 7    * * *   /cache/drake-ci/cache_server/disk_usage_alert.py /cache/data >>/cache/log/disk_usage_alert.log 2>&1
    #
...
continuous) will be littered with "Internal 500 Errors".  `bazel` tried to do
an HTTP `PUT`, but the server did not have the ability to store it.  While the
`bazel build` will continue without error, this means that our cache is getting
out of sync and cleanup should happen quickly.  After cleanup, it may be worth
checking the logs in `/cache/log/` for more data on when the cache server disk
utilization started growing too quickly.  Perhaps the nightly cleanup routine
...
the server and runs [`disk_usage_alert.py`](./disk_usage_alert.py) to make sure
there is enough space available to upload new cache entries.  If either one of
these fails, buildcops will receive an email via Jenkins.
