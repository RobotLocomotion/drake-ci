...
3. Each server should have three `cron` jobs running to expunge old files
   using [`expunge_old_files.py`](./expunge_old_files.py), monitor the disk
...
11. Confirm that `echo $USER` reveals you are `root`, and then execute
    ...
    0 22 * * *   /cache/drake-ci/cache_server/expunge_old_files.py -s --days 3 /cache/data >>/cache/log/expunge_old_files.log 2>&1
    #
