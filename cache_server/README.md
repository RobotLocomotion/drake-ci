...
In the event that the `cron` jobs running
[`remove_old_files.py`](./remove_old_files.py) is not removing enough disk space
the monitoring jobs in the previous section will start notifying the buildcops.
You must log in to the cache server in question and run the script manually with
new arguments for the window of time to consider.
...
days (`--days 1 --hours 12`) should be safe to delete immediately.
