...
After ``logrotate`` runs, however, there are two issues for ``nginx`` logging that must
be corrected.

1. The ``nginx`` logging files ``/opt/cache_server/nginx/{access,error}.log`` may have
   been deleted.  These files are **not** recreated by ``nginx``.  Recreating them
   with the correct permissions and restarting the ``nginx`` service seem to enable
   continued logging with ``nginx``.
2. Even when a file such as ``/opt/cache_server/nginx/access.log.1`` (an ``access.log``
   that was rotated once previously) is deleted, the disk space under ``/`` does not
   get freed.  Restarting the ``nginx`` service seems to free the space.
"""
